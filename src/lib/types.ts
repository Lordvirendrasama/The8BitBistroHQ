...
export interface LiabilityState {
  id: string;
  loanPrincipalStart: number;
  loanBalance: number;
  annualInterestRate: number; // 9 by default
  loanStartDate: string;
  totalLoanPaid: number;
  totalInterestPaid: number;
  
  monthlyRent: number; // 50000 by default
  rentBalance: number;
  totalRentPaid: number;
  
  lastLiabilityCycleUpdate: string; // ISO Date string of last applied monthly cycle
  trackingStartDate: string; // When the system started tracking behavior
}
