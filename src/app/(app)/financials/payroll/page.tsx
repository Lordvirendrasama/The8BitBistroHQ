...
export default function PayrollPage() {
  ...
  if (loading) return <div className="p-12 text-center font-headline text-sm animate-pulse opacity-50">Syncing Workforce Data...</div>;
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-muted-foreground font-bold uppercase text-sm tracking-normal">WAGE CALCULATION BASED ON INDIVIDUAL EMPLOYEE SALARIES & ATTENDANCE</p>
        </div>
        <div className="flex gap-2">
            <Select value={selectedMonth} onValueChange={setSelectedMonth}>
                <SelectTrigger className="w-[200px] h-12 font-bold uppercase text-sm border-2">
                    <SelectValue />
                </SelectTrigger>
                <SelectContent>
                    {monthOptions.map(m => (
                        <SelectItem key={m} value={m} className="font-bold uppercase text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-2 bg-muted/5">
            ...
            <CardContent>
                <div className="text-3xl font-bold">₹{settings.hourlySalaryRate || 100}<span className="text-sm ml-1 opacity-40">/HR</span></div>
                <p className="text-sm font-bold text-muted-foreground uppercase mt-1">Used if individual salary is unset</p>
            </CardContent>
        </Card>
        ...
        <Card className="border-2 bg-muted/5">
            ...
            <CardContent>
                <div className="text-3xl font-bold">{stats.length} <span className="text-sm ml-1 opacity-40">OPERATORS</span></div>
                <p className="text-sm font-bold text-muted-foreground uppercase mt-1">Total workforce tracked</p>
            </CardContent>
        </Card>
        ...
        <Card className="border-2 bg-primary/5 border-primary/20">
            ...
            <CardContent>
                <div className="text-3xl font-bold text-primary">₹{Math.floor(stats.reduce((s, st) => s + st.totalEarnings, 0)).toLocaleString()}</div>
                <p className="text-sm font-bold text-muted-foreground uppercase mt-1">Total estimated wages for selected period</p>
            </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-2 shadow-none overflow-hidden">
        ...
        <CardContent className="p-0">
            <Table>
                <TableHeader>
                    <TableRow>
                        <TableHead className="font-bold uppercase text-sm pl-6">Operator</TableHead>
                        <TableHead className="font-bold uppercase text-sm text-center">Structure</TableHead>
                        <TableHead className="font-bold uppercase text-sm text-center">Days / Hours</TableHead>
                        <TableHead className="font-bold uppercase text-sm text-center">Late Logs</TableHead>
                        <TableHead className="text-right font-bold uppercase text-sm pr-6">Earned Salary</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {stats.map((staff) => {
                            ...
                            <TableRow key={staff.id} className="hover:bg-muted/5">
                                <TableCell className="pl-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <Avatar className="h-8 w-8 border-2 border-primary/10">
                                            <AvatarFallback className="text-sm font-bold">{staff.displayName[0]}</AvatarFallback>
                                        </Avatar>
                                        <div className="flex flex-col">
                                            <span className="font-bold uppercase text-sm">{staff.displayName}</span>
                                            <span className="text-sm font-bold text-muted-foreground uppercase">@{staff.id}</span>
                                        </div>
                                    </div>
                                </TableCell>
                                <TableCell className="text-center">
                                    <Badge variant="outline" className="h-4 text-sm font-bold uppercase border-emerald-500/20 text-emerald-600">
                                        {staff.salaryType} (₹{staff.baseRate.toLocaleString()})
                                    </Badge>
                                </TableCell>
                                <TableCell className="text-center">
                                    <div className="flex flex-col items-center justify-center font-mono text-sm font-bold">
                                ...
                                <TableCell className="text-center">
                                    {staff.lateInstances > 0 ? (
                                        <Badge variant="destructive" className="h-4 text-sm font-bold uppercase">{staff.lateInstances} TIMES</Badge>
                                    ) : <Badge variant="outline" className="h-4 text-sm font-bold uppercase text-emerald-600 border-emerald-500/20">PERFECT</Badge>}
                                </TableCell>
                                <TableCell className="text-right pr-6">
                                    <span className="font-mono font-bold text-lg text-primary">₹{Math.floor(staff.totalEarnings).toLocaleString()}</span>
                                </TableCell>
                            </TableRow>
                        );
                    })}
                    {stats.length === 0 && (
                        <TableRow>
                            <TableCell colSpan={5} className="h-64 text-center opacity-30 italic font-bold uppercase text-sm tracking-normal">No staff operators found in database.</TableCell>
                        </TableRow>
                    )}
                </TableBody>
            </Table>
