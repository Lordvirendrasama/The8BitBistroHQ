...
export default function StaffPage() {
    ...
    useEffect(() => {
        if (user && db && (user.role === 'staff' || user.role === 'admin')) {
    ...
    const handleLogoutClick = async () => {
        if (user && (user.role === 'staff' || user.role === 'admin') && activeShift) {
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-muted-foreground">
                        {user?.role === 'guest'
                            ? 'Viewing the daily operational checklist.'
                            : activeShift
                    ...
                    <div className="space-y-4">
                        {recentShifts && recentShifts.length > 0 ? (
                            ...
                            <p className="text-center text-muted-foreground py-8">
                                {user?.role === 'guest' ? 'Guests do not have shifts.' : 'No completed shifts found.'}
                            </p>
                        )}
                    </div>
                </CardContent>
