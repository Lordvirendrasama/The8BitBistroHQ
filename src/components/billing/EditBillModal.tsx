      ...
      <DialogContent className="max-w-[95vw] md:max-w-6xl h-[95vh] md:h-[85vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl">
        <DialogHeader className="p-4 md:p-6 bg-muted/10 border-b shrink-0">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm font-bold text-yellow-600 uppercase flex items-center gap-1.5">
          ...
          <div className="flex-1 flex flex-col border-b md:border-b-0 md:border-r overflow-hidden">
            <div className="p-3 bg-muted/20 border-b space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm uppercase tracking-normal">Bistro Menu</h3>
              </div>
          ...
          <div className="w-full md:w-[420px] flex flex-col bg-card overflow-hidden">
            <div className="p-3 bg-muted/20 border-b flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm uppercase tracking-normal">Current Bill Audit</h3>
              </div>
              ...
              <Table>
                <TableBody>
                  {billItems.length > 0 ? billItems.map((item, idx) => (
                    ...
                    <TableRow>
                        <TableCell colSpan={3} className="text-center py-20 text-sm italic font-bold text-muted-foreground uppercase opacity-40">No items in bill</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
            ...
            <div className="p-4 bg-muted/20 border-t space-y-4 shrink-0">
                <div className="space-y-2">
                    <h4 className="font-bold text-sm uppercase tracking-normal text-muted-foreground/60 border-b pb-1">Payment Method & Adjustments</h4>
                    ...
                    <RadioGroup value={paymentMethod} onValueChange={(v) => setPaymentMethod(v as PaymentMethod)} className="grid grid-cols-2 gap-2">
                        ...
                        <Label className={cn("flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all", paymentMethod === 'split' ? "bg-amber-500/10 border-amber-500 text-amber-700" : "bg-background border-muted")}>
                            ...
                            <span className="text-sm font-bold uppercase">Split</span>
                        </Label>
                        <Label className={cn("flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all", paymentMethod === 'district-dinein' ? "bg-amber-500/10 border-amber-500 text-amber-700" : "bg-background border-muted")}>
                            ...
                            <span className="text-sm font-bold uppercase">District</span>
                        </Label>
                    ...
                    {paymentMethod === 'split' && (
                        <div className="grid grid-cols-2 gap-2 p-2 bg-background border-2 border-dashed rounded-lg animate-in slide-in-from-top-2 duration-200">
                            ...
                            {!isSplitValid && (
                                <p className="col-span-2 text-sm font-bold text-destructive uppercase text-center mt-1">Warning: Totals do not match (Sum: ₹{splitTotal})</p>
                            )}
                        </div>
                    )}
                    ...
                    {initialPackagePrice > 0 && (
                        <div className="flex justify-between items-center text-sm font-bold uppercase text-muted-foreground tracking-normal">
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                            <Label htmlFor="discount-edit" className="text-sm font-bold uppercase tracking-normal flex items-center gap-1"><Tag className="h-3 w-3 text-destructive"/> Discount</Label>
                            <Input 
                                id="discount-edit"
                                type="number"
                                value={discount || ''}
                ...
                <div className="flex justify-between items-center">
                    <span className="text-sm font-bold uppercase tracking-[0.2em] text-muted-foreground">Final Grand Total</span>
        ...
        <DialogFooter className="p-4 bg-background border-t shrink-0 flex flex-col sm:flex-row gap-2">
          ...
          <Button onClick={handleSave} disabled={paymentMethod === 'split' && !isSplitValid} className="w-full sm:flex-[2] font-bold uppercase text-sm h-12 shadow-xl tracking-normal">
